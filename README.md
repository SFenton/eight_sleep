...
| Now Level               | Side  | Number      | Yes      | The app-scale explicit current/tonight bedtime override level (-10 to 10). When there is no active override, the state is Unknown and `has_override` is false. The raw API value is available as the `raw_value` attribute. |
| Active Level            | Side  | Number      | Yes      | The app-scale active target heating/cooling level (-10 to 10). This follows the side's current API target and may change when smart mode turns on. The raw API value is available as the `raw_value` attribute. |
| Bedtime Level           | Side  | Number      | Yes      | The app-scale smart schedule bedtime level (-10 to 10). The raw API value is available as the `raw_value` attribute.                                                                            |
| Asleep Level            | Side  | Number      | Yes      | The app-scale smart schedule initial/asleep level (-10 to 10). The raw API value is available as the `raw_value` attribute.                                                                     |
| Dawn Level              | Side  | Number      | Yes      | The app-scale smart schedule final/dawn level (-10 to 10). The raw API value is available as the `raw_value` attribute.                                                                         |
